...
public class GdLotTransCodTransport implements Transport {
	...
	private static Logger logger = LoggerFactory
			.getLogger(GdLotTransCodTransport.class);
	...
	@Override
	public Object submit(Object paramRequest, Context context)
		...
		try {
			...
			logger.info("this.b=" + this.b + ",this.c=" + c + ",this.gateway="
					+ this.gateway);
			// localObject2 =
			// transform(paramRequest,this.b,context.getProcessId());//待发送报文字节数组
		...
		if (localObject3 != null) {
			try {
				// localObject1 =
				// transform(localObject3,this.c,context.getProcessId());
	 ...
	 * @param paramObject
	 *            :初始字节数组
	 * @param paramArrayOfTransform
	 *            :配置文件
	 * @param paramString
	 *            :交易码
	 * @return:组装完成的报文字节数组
	 */
	protected Object transform(Object paramObject,
			Transform[] paramArrayOfTransform, String paramString) {
		...
		if (paramArrayOfTransform != null) {
			...
			for (int i = 0; i < atransform.length; i++) {
				...
				try {
					localObject = localTransform.transform(localObject,
							paramString);
	...
	protected Object encode(Context context) {
		...
		try {
			...
			w.writeCharacters(DateUtils.format(new Date(),
					"yyyy-MM-dd HH:mm:ss"));
			w.writeEndElement();
			w.writeEndElement();
			...
			} else if ("232".equals(action)) { // 实时单笔投注查询
				action_232(w, context);
			}
	...
	private void action_232(XMLStreamWriter w, Context context)
			throws XMLStreamException {

		// <pkgC>
		// <dealer_serial>交易流水号</dealer_serial>
		// <gambler_name>彩民标识(见编码类型值表)</gambler_name>
		// </pkgC>
	...
	private void action_238(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
	...
	private void action_201(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
	...
	private void action_219(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
		w.writeStartElement("gambler_name");
		w.writeCharacters(context.getData("lotNam").toString());
	...
	private void action_235(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
		w.writeStartElement("game_id");
		w.writeCharacters(context.getData("gameId").toString());
	...
	private void action_212(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
	...
	private void action_209(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
		w.writeStartElement("gambler_name");
		w.writeCharacters(context.getData("gambler_name").toString());
	...
	private void action_236(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
		w.writeStartElement("game_id");
		w.writeCharacters(context.getData("gameId").toString());
	...
	private void action_240(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
		w.writeStartElement("game_id");
		w.writeCharacters(context.getData("gameId").toString());
	...
	private void action_237(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
		w.writeStartElement("game_id");
		w.writeCharacters(context.getData("gameId").toString());
	...
	private void action_231(XMLStreamWriter w, Context context)
			throws XMLStreamException {
		w.writeStartElement("pkgC");
	...
	protected Object decode(Object obj, Context context) {
		...
		try {
			...
			String body = data.substring(data.indexOf("<pkgC>"),
					data.indexOf("</pkg>"));
			...
			} else if ("232".equals(action)) {
				decode_232(r, map, context);
			}
	...
	private void decode_232(XMLStreamReader r, Map<String, Object> map,
		...
		while (r.hasNext()) {
			...
			if ("schemeInfo".equals(name)) {
				if ("dealer_serial".equals(name)) { // 交易流水号
					map.put("txnLog", r.getElementText());
				}
				if ("scheme_id".equals(name)) { // 系统生成的方案编号
					map.put("schId", r.getElementText());
				}
				if ("serial_no".equals(name)) { // 彩票流水号
					map.put("tLogNo", r.getElementText());
				}
				if ("cipher".equals(name)) { // 彩票密码
					map.put("cipher", r.getElementText());
				}
				if ("checksum".equals(name)) { // 彩票校验码
					map.put("verify", r.getElementText());
				}
				if ("scheme_type".equals(name)) { // 方案类型(见编码类型值表)
					map.put("schTyp", r.getElementText());
				}
				if ("scheme_title".equals(name)) { // 方案标题
					map.put("schTit", r.getElementText());
				}
				if ("secrecy_level".equals(name)) { // 方案等级(见编码类型值表)
					map.put("secLev", r.getElementText());
				}
				if ("create_time".equals(name)) { // 生成时间
					map.put("lotTxnTim", r.getElementText());
				}
				if ("city_id".equals(name)) { // 地市编号
					map.put("cityId", r.getElementText());
				}
				if ("game_id".equals(name)) { // 游戏
					map.put("gameId", r.getElementText());
				}
				if ("draw_id".equals(name)) { // 当前期
					map.put("drawId", r.getElementText());
				}
				if ("keno_draw_id".equals(name)) { // 快开游戏小期
					map.put("kenoId", r.getElementText());
				}
				if ("play_id".equals(name)) { // 玩法编号
					map.put("playId", r.getElementText());
				}
				if ("bet_method".equals(name)) { // 投注方法(机选\自选\...)
					map.put("betTyp", r.getElementText());
				}
				if ("bet_mode".equals(name)) { // 投注方式
					map.put("betMod", r.getElementText());
				}
				if ("bet_multiple".equals(name)) { // 投注倍数
					map.put("betMul", r.getElementText());
				}
				if ("bet_money".equals(name)) { // 投注金额
					map.put("betAmt", r.getElementText());
				}
				if ("betInfo".equals(name)) {
					map.put("grpNum", r.getAttributeValue(0)); // 号码组个数
					map.put("betNum", r.getAttributeValue(1)); // 总号码个数
					if ("bet_line".equals(name)) { // 投注号码
						map.put("betLin", r.getElementText());
					}
				}
				if ("status".equals(name)) { // 方案状态
					map.put("schSts", r.getElementText());
				}
				if ("prize_money".equals(name)) { // 中奖金额
					map.put("bonAmt", r.getElementText());
				}
			}
		}
	...
	private void decode_238_2(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
	...
	private void decode_238_3(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		List<Map> list = new ArrayList<Map>();
		List<Map> lst = new ArrayList<Map>();
		Map m = new HashMap();
			...
			if (StringUtils.isNotEmpty(gameId)) {
				if ("7".equals(gameId)) {
					...
					if ("scheme".equals(name)) {
						...
						mm.put("chkDat", DateUtils.format(new Date(),
	...
	private void decode_238(XMLStreamReader r, Map<String, Object> map,
			Context context) throws XMLStreamException {
	...
	private void decode_201(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		while (r.hasNext()) {
			int typ = r.next();
			if (typ != XMLStreamConstants.START_ELEMENT)
	...
	private void decode_235(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
	...
	private void decode_212(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		while (r.hasNext()) {
			int typ = r.next();
			if (typ != XMLStreamConstants.START_ELEMENT)
	...
	private void decode_200(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		while (r.hasNext()) {
			int typ = r.next();
			if (typ != XMLStreamConstants.START_ELEMENT)
	...
	private void decode_219(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		while (r.hasNext()) {
			int typ = r.next();
			if (typ != XMLStreamConstants.START_ELEMENT)
	...
	private void decode_231(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		while (r.hasNext()) {
			int typ = r.next();
			if (typ != XMLStreamConstants.START_ELEMENT)
	...
	private void decode_209(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		while (r.hasNext()) {
			int typ = r.next();
			if (typ != XMLStreamConstants.START_ELEMENT)
	...
	private void decode_236(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
	...
	private void decode_240(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
	...
	private void decode_237(XMLStreamReader r, Map<String, Object> map)
			throws XMLStreamException {
		while (r.hasNext()) {
			int typ = r.next();
			if (typ != XMLStreamConstants.START_ELEMENT)
