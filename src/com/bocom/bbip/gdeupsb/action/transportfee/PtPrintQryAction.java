...
public class PtPrintQryAction extends BaseAction {
	...
	public void execute(Context ctx) throws CoreException, CoreRuntimeException {
		...
		List<Integer> num = gdeupsTyfInvRecRepository
				.findCount(gdeupsTyfInvRec);
		if (num.get(0) == 0) {
			throw new CoreException(ErrorCodes.EUPS_CONSOLE_INFO_NOTEXIST);
		}

		Pageable pageable = BeanUtils.toObject(ctx.getDataMap(),
				PageRequest.class);
		Page<GdeupsTyfInvRec> invInfoPage = get(GdeupsTyfInvRecRepository.class)
				.find(pageable, gdeupsTyfInvRec);
