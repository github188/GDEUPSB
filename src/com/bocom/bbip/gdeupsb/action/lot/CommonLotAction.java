...
public class CommonLotAction extends BaseAction {
    ...
    /**
     * 根据与福彩之间的时差获取福彩时间 INPUT: brNo:分行号 dealId:福彩时间 lclTim:本地时间
     * 
     * @return fcTim 福彩时间
     */
    ...
    /**
     * 计算时差 INPUT: nodNo:网点号 brNo:分行号 lotTim:福彩时间 lclTim:本地时间
     * 
     * @return difTim:时差（秒）
     */
    ...
    /**
     * 计算时间 INPUT: brNo:分行号 srcDate:原时间 difTim:时差（秒）
     * 
     ...
     * 
     * @param FileType
     *            文件类型（1：游戏规则文件分类 2：中奖记录文件分类 3:对账文件分类）
     * @param gameId
     *            游戏编号
     * @param drawId
     *            期号
    ...
    /**
     * 计算轧差 CalStat 轧差状态（0：成功 -1：失败） CalMsg:轧差结果描述
     */
            ...
            if (drwTblMap.get("unPrzCnt") == "0") {
                ...
                if (sumPrzDrwMap.get("przSumAmt") != "0") {
                    ...
                    try {
                        ...
                        context.setData("calStat", "-1");
                        context.setData("calMsg",
                                "更新汇总返奖信息失败[" + gdLotDrwTbl.getGameId() + "-" + gdLotDrwTbl.getDrawId() + "]");
    ...
    public void GetSysCfg(Context context) {
        ...
        // TODO; 测试
        // Result dsResult =
        // bgspServiceAccessObject.callServiceFlatting("queryCorporInfo",
        // inpara);
