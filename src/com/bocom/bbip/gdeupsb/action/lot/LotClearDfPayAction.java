 ...
 * 
 */
public class LotClearDfPayAction extends BaseAction {

	@Override
	public void execute(Context context) throws CoreException, CoreRuntimeException {
		log.info("LotClearDfPayAction start!..");
		...
		if (!"0".equals(payAmt)) {
			...
			inmap.put("tfiCusAc", hsActNo); // 收款帐号
			inmap.put("tfiCusNme", " "); // 收款帐号户名
			inmap.put("fudDir", "1"); // 转出方向为转对公账户
			inmap.put("tfaAmt", payAmt); // 转账金额
			inmap.put("rma", "还返奖垫付资金"); // 备注
			try {
...
//					throw new CoreException("还垫付款失败[返回失败]");
...
//					context.setState(BPState.BUSINESS_PROCESSNIG_STATE_FAIL);
//
//					throw new CoreException("还垫付款失败[系统错误]");
...
//					throw new CoreException("还垫付款失败[发送超时],请及时查询会计流水!");
//				} else {
//					context.setData(ParamKeys.RESPONSE_TYPE, Constants.RESPONSE_TYPE_FAIL);
//					context.setData(ParamKeys.RESPONSE_CODE, ErrorCodes.TRANSACTION_RESPONSE_ISNULL);
//					context.setState(BPState.BUSINESS_PROCESSNIG_STATE_UNKOWN_FAIL);
//					throw new CoreException("还垫付款失败[其他错误]");
//				}
