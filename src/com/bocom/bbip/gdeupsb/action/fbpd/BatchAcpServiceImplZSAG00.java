...
public class BatchAcpServiceImplZSAG00 extends BaseAction implements
	...
	public void prepareBatchDeal(PrepareBatchAcpDomain domain, Context context)
		...
		if (context.getData("mothed").equals("1")) {
			...
			String path = "./save/tfiles/" + context.getData(ParamKeys.BR)
...
//			config.setLocDir("D:/test/ZSAG00/");// TODO
		...
		Assert.isFalse(null == result || 0 == result.size(),
				ErrorCodes.EUPS_FILE_PARESE_FAIL, "解析文件出错");
		...
		if ("4840000015".equals(comNo) || "4840000167".equals(comNo)
				...
				|| "4840000598".equals(comNo) || "4840000414".equals(comNo)
				|| "4840000374".equals(comNo)) { // OTHER
			List<GdFbpdObusBatchTmp> list = (List<GdFbpdObusBatchTmp>) BeanUtils
			...
			((SqlMap) get("sqlMap"))
					.insert("com.bocom.bbip.gdeupsb.entity.GdFbpdObusBatchTmp.batchInsert",
							list);

			logger.info("~~~~~~~End  ~~~~将数据插入临时表");
			List<GdFbpdObusBatchTmp> lt = gdFbpdObusBatchTmpRepository
		...
		if ("4840000016".equals(comNo) || "4840000484".equals(comNo)
			...
			// NELE
			List<GdFbpdNeleBatchTmp> list = (List<GdFbpdNeleBatchTmp>) BeanUtils
			...
			((SqlMap) get("sqlMap"))
					.insert("com.bocom.bbip.gdeupsb.entity.GdFbpdNeleBatchTmp.batchInsert",
							list);
			logger.info("~~~~~~~End  ~~~~将数据插入临时表");
			List<GdFbpdNeleBatchTmp> lt = gdFbpdNeleBatchTmpRepository
					.findByBatNo((String) context.getData(ParamKeys.BAT_NO));
			// for(GdEupsFbpdObusBatchTmp temp:lt){
			// temp.setCusAc(temp.getCusAc());
			// temp.setCusNme(temp.getCusNme());
		...
		if ("4840000416".equals(comNo)) {
			...
			List<GdFbpdMposBatchTmp> list = (List<GdFbpdMposBatchTmp>) BeanUtils
			...
			// gdEupsZHAGBatchTempRepository.batchInsert(list);
			((SqlMap) get("sqlMap"))
					.insert("com.bocom.bbip.gdeupsb.entity.GdFbpdMposBatchTmp.batchInsert",
							list);
			logger.info("~~~~~~~End  ~~~~将数据插入临时表");
			List<GdFbpdMposBatchTmp> lt = gdFbpdMposBatchTmpRepository
					.findByBatNo((String) context.getData(ParamKeys.BAT_NO));
			// for(GdEupsFbpdObusBatchTmp temp:lt){
			// temp.setCusAc(temp.getCusAc());
			// temp.setCusNme(temp.getCusNme());
