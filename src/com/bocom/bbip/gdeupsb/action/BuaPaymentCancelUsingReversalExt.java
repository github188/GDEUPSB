 ...
 * 
 * @version 1.0.0,2013-4-22
 * @author sean.wang
 * @since 1.0.0
 */
public class BuaPaymentCancelUsingReversalExt extends BaseAction {

    /** Logger. */
    private static final Log logger = LogFactory.getLog(BuaPaymentCancelUsingReversalExt.class);

    public void execute(Context context) throws CoreException, CoreRuntimeException {
        logger.info("Enter in BuaPaymentCancelUsingReversalExt component...");
...
//        }
    		...
    		String sqnHst=get(BBIPPublicService.class).getBBIPSequence();
//        	context.setData("mfmVchNo", sqnHst.substring(sqnHst.length()-10));
        	String sqnJrn=get(BBIPPublicService.class).getBBIPSequence();
//        	context.setData("mfmJrnNo", sqnJrn.substring(sqnJrn.length()-10));
        	
    	        context.setData(ParamKeys.AGTS_ACCOUNT_VOUCHER_NO, sqnHst.substring(sqnHst.length()-10));
    	        context.setData(ParamKeys.AGTS_ACCOUNT_DATE, new Date());
    	        context.setData(ParamKeys.AGTS_BBIP_JOURNAL_NO, sqnJrn);
    	        context.setData(ParamKeys.MFM_RSP_MSG, "冲正成功");
    	        logger.info("Call BBIP payment service response successful.");
                context.setState(BPState.BUSINESS_PROCESSNIG_STATE_NORMAL);
                context.setData(ParamKeys.REVERSE_RESULT_CODE, Constants.RESPONSE_CODE_SUCC_HOST);
                context.setData(ParamKeys.REVERSE_RESULT_MESSAGE, "冲正成功");
                logger.info("call host reversal suc!context=" + context.getDataMap());
