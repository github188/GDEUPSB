...
/**
 * 用户协议文件上传（0全部，1新增） 生成文件hdCNJTyyyyMMdd.txt、rxyCNJTyyyyMMdd.txt
...
public class GasAgentBatchFileToFtpAction extends BaseAction {
	...
	private static Logger logger = LoggerFactory
			.getLogger(GasAgentBatchFileToFtpAction.class);
	...
	public void execute(Context context) throws CoreException {
		...
		if (StringUtils.isNotEmpty(fileDte)) { // 交易日期非空，手动发起
			...
			fileDte = DateUtils.format(getYesterDay(),
		...
		/*
		 * FTP配置 1、从FTP配置表中获取信息 2、修改文件名对应字段
		 */

		String gasAgtFilCfgA = "PGAS00AgtA"; // FTP_NO
		String gasAgtFilCfgB = "PGAS00AgtB";

		EupsThdFtpConfig ftpConfigA = get(EupsThdFtpConfigRepository.class)
				.findOne(gasAgtFilCfgA);
		EupsThdFtpConfig ftpConfigB = get(EupsThdFtpConfigRepository.class)
				.findOne(gasAgtFilCfgB);
		// TODO 文件路径（本地，远程）
		...
		try {
			// 生成文件到指定路径
			get(OperateFileAction.class).createCheckFile(ftpConfigA,
					"gasAllAgentBatFmt", hdFileName, hdMap);
			logger.info("hdCNJTyyyyMMdd.txt文件生成成功！");
			// get(OperateFileAction.class).createCheckFile(ftpConfigB,
			// "gasAllAgentBatFmt", hdFileName, hdMap);
...
//		get(OperateFTPActionExt.class).putCheckFile(ftpConfigB);
		...
		try {
			// 生成文件到指定路径
			get(OperateFileAction.class).createCheckFile(ftpConfigA,
					"gasDayAgentBatFmt", rxyFileName, rxyMap);
...
//		get(OperateFTPActionExt.class).putCheckFile(ftpConfigB);
	 ...
	 */
	private Map<String, Object> encodeAgtHdFileMap(String optDat)
			throws CoreException, CoreRuntimeException {
		Map<String, Object> map = new HashMap<String, Object>();

		GdGasCusAll cusAllAgt = new GdGasCusAll();
		cusAllAgt.setOptDat(optDat);
		List<GdGasCusAll> cusAllList = get(GdGasCusAllRepository.class)
...
//			throw new CoreException(ErrorCodes.EUPS_QUERY_NO_DATA);
	 ...
	 */
	private Map<String, Object> encodeAgtRxyMap(String optDat)
			throws CoreException, CoreRuntimeException {
		Map<String, Object> map = new HashMap<String, Object>();
		GdGasCusDay cusDayAgt = new GdGasCusDay();
		cusDayAgt.setOptDat(optDat);
		List<GdGasCusDay> cusDayList = get(GdGasCusDayRepository.class).find(
				cusDayAgt);
