...
public class BatchSignFileInputAction extends BaseAction {
	...
	public void fileInput(Context context) throws CoreException, CoreRuntimeException {
		...
		/*
		 * if(filNm.length()!=29||!filNm.substring(5,6).equals("_")||!filNm.
		 * substring
		 * (14,15).equals("_")||!filNm.substring(21,22).equals("_")||!filNm
		 * .substring(filNm.length()-4,filNm.length()).equals(".TXT")){
		 * log.info("check fileName rule error...... "); throw new
		 ...
		 * log.info("check fileName BR error...... "); throw new
		 * CoreException(GDErrorCodes.EUPS_FILE_HEADBR_ERROR); }
		 */
		// ======登录FTP开始下载文件到本地======
		// 取FTP信息
		...
		for (int i = 0; i < detailList.size(); i++) {
			...
			if (strGdsBId.equals("44103") || strGdsBId.equals("44102")) {
				...
				inpara.put("agtstb", agtstb); // 子表名称
				inpara.put("lagtSt", (String) context.getData("status")); // 协议校验状态
				inpara.put("actNo", actno); // 卡号
				inpara.put("gdsBid", gdsBid); // 具体业务类型
				inpara.put("tcusId", tcusid); // 客户标志
				inpara.put("lerMsg", (String) context.getData("retmsg")); // 校验错误消息
			...
			try {
				...
				//存入子表消息
				log.info("开始存入子表消息");
				gdsAgtWaterRepository.updateStbInf(inpara);
	...
	// 报表打印 BBIPIBV0001
	/*
	 * public void reportPrint(Context context) throws CoreException,
	...
	public void reportPrint(Context context) throws CoreException, CoreRuntimeException {
		...
		if (CollectionUtils.isNotEmpty(gdsbatagtinfList)) {
			...
			if (CollectionUtils.isNotEmpty(gdsbatagtinfList)) {
				...
				String reportName = str.append("GDS").append(gdsBId).append("_").append(context.getData(ParamKeys.TELLER)).append("_SUCC.RPT")
						.toString();
			...
			if (CollectionUtils.isNotEmpty(gdsbatagtinflist)) {
				Map<String, String> map = new HashMap<String, String>();
				StringBuffer str = new StringBuffer();
				String reportName = str.append("GDS").append(gdsBId).append("_").append(context.getData(ParamKeys.TELLER)).append("_Fail.RPT")
						.toString();
	...
	public void preDealCheck(Context context)
		...
		if (cusActInfResult.isSuccess()) {
			if (!cusActInfResult.getIdNo().equals(idNo)) {
				...
				log.info("在此处，context="+context.getDataMap());
...
//			context.setData("isContinue", "N");
	...
	/*
	 * private void renderActingCusReceiptFile(List<Gdsbatagtinf>
	 * gdsbatagtinfList,Context context)throws
	 * CoreException,CoreRuntimeException {
	 ...
	 * context.setData("gdsBid", context.getData("gdsBid"));//企业种类 Date
	 * appealTime = new Date(); String strDate = DateUtils.format(appealTime,
	 * "yyyyMMdd"); //打印日期 context.setData("prntDat",strDate.substring(0, 4));
	 * context.setData("prntMoth",strDate.substring(4, 2));
	 * context.setData("prntDay",strDate.substring(6, 2));
	 * 
	 * String reportFileName =context.getData("reportName") ;// 打印报表文件名 String
	 * reportId=context.getData("reportId"); try {
	 * GdEupsbUtils.renderReportFile(context, reportRender, mftpTransfer,
	 * reportId, reportFileName, localDir, bbosDir); } catch
	...
	/*
	 * public void agtSignDeal(Context context,GdsAgtInf gdsAgtInf) throws
	 * CoreException, CoreRuntimeException{ //====查询签约主表信息是否存在该记录
	 * 
	 * GdsAgtInf gdsagtinf=new GdsAgtInf(); Gdsagtunca gdsagtunca =new
	 * Gdsagtunca(); Gdsagtmob gdsagtmob =new Gdsagtmob(); String
	 * actno=gdsAgtInf.getActNo(); String gdsBid=gdsAgtInf.getGdsBid();
	 * gdsagtinf.setActNo(actno); gdsagtinf.setGdsBid(gdsBid); //=====移动===
	 * gdsagtmob.setActno(actno); gdsagtmob.setGdsbid(gdsBid); //===联通===
	 * gdsagtunca.setActno(actno); gdsagtunca.setGdsbid(gdsBid); List
	 * <GdsAgtInf> gdsAgtInflist=null; //联通 List <Gdsagtunca> gdsagtuncalist
	 * =null; //移动 List <Gdsagtmob> gdsagtmoblist=null; try{
	 * gdsAgtInflist=gdsAgtInfRepository.find(gdsagtinf);
	 * 
	 * }catch(Exception CoreException){//查询协议表出错
	 * 
	 * } ///===不为空修改数据 if(CollectionUtils.isNotEmpty(gdsAgtInflist)){ try{
	 * gdsAgtInfRepository.updateOldAgtInfo(BeanUtils.toMap(gdsAgtInf));
	 * 
	 * }catch(Exception CoreException){
	 * 
	 * 
	 * } //===查询字表是否有数据44103 44102 if(gdsBid=="44102"){ //查询联通字表是否有数据 try{
	 * gdsagtuncalist= gdsagtuncaRepository.find(gdsagtunca); }catch(Exception
	 * CoreException){
	 * 
	 * } //===存在更新 if(CollectionUtils.isNotEmpty(gdsagtuncalist)){
	 * gdsagtuncaRepository.s }
	 * 
	 * }else if(gdsBid=="44103"){ //查询移动字表是否有数据 try{
	 * gdsagtmoblist=gdsagtmobRepository.find(gdsagtmob); }catch(Exception
	 * CoreException){
	 * 
	 * } }else if(gdsBid==""){
	 * 
	 ...
	 * 
	 * 
	 * } //为空新增数据 if(CollectionUtils.isEmpty(gdsAgtInflist)){
	 * gdsAgtInf.setIvdDat("99991231");
	 * gdsAgtInf.setEffDat((String)context.getData("strDate"));
	 * gdsAgtInfRepository.insert(gdsAgtInf); } //------处理子表记录，将所有记录置"无效" }
	 */
}
