...
public class CnlPayUnilateralToBankServicePGAS00 implements
	 ...
	 * 
	 * // 根据燃气托收流水、账务状态（正常扣款）、已扣费用、返回第三方状态（B0扣款成功）、主机返回码（SC0000）、主机交易状态（S）、
	 * 第三方交易码查询流水表 // CTD会传： 交易码thdTxnCde、托收流水thdSqn、银行标识BK、冲正金额txnAmt // select
	 * * from Gastxnjnl491 where CLogNo='%s' and Status='1' and // OptAmt='%s'
	...
	public Map<String, Object> prepareCheckDeal(CommHeadDomain commHeadDomain,
		...
		logger.info("CnlPayUnilateralToBankServicePGAS00@prepareCheckDeal start!");
		...
		logger.info("CnlPayUnilateralToBankServicePGAS00@prepareCheckDeal end!");
		return null;
	}
